...
public class AlgAnalysis {
	...
	public static void arraySortRuntime(int count, int maxSize){
		int increment = maxSize / count;
		for (int i = increment; i < maxSize + 1; i = i + increment) {

			double worstCase = 0.0;
			// maxSize / count ; figures out how many arrays would be made.
			...
			for (int j = 0; j <10 ; j ++) {
				...
				double result = nanoToSeconds(time2 - time1);
				if (result > worstCase) {
					worstCase = result;
					}
			}
			...
			System.out.println();
			System.out.println("n = " + (i) + "; T(n) = " + Double.toString(worstCase) + "; T(n)/(n * n) = " +
			 Double.toString(worstCase / (i * i)) + "; T(n)/(n * log n) = " + 
			 Double.toString(worstCase / (i * Math.log(i)))+".");
	...
	public static long unique1Runtime(int n){
		...
		int[] testArray = randomizeArray(genArray(n));
		
		Boolean isUnique1 = false;
		Long timeStart = 0L;
		Long timeEnd = 0L;
		while (isUnique1 == false) {
			timeStart = System.nanoTime();
			isUnique1 = Unique1.unique1(testArray); //used to just call the method in general so that the time can pass by.
	...
	public static long unique2Runtime(int n){
		...
		Long timeStart = 0L;
		Long timeEnd = 0L;
		while (isUnique2 == false) {
			timeStart = System.nanoTime();
			isUnique2 = Unique2.unique2(testArray); //used to just call the method in general so that the time can pass by.
			timeEnd = System.nanoTime();
		}
